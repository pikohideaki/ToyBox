...
class CGame {
	...
	/* カード移動基本操作 */
	GetCardWithID( card_ID, remove_this_card = true ) {
		let card = new CCard();
		let matched_num = 0;
		const G = this;
		...
		for ( let i = 0; i < G.Players.length; ++i ) {
			...
			for ( let k = 0; k < pl.Deck.length; ++k ) {
				if ( card_ID === pl.Deck[k].card_ID ) {
					card = pl.Deck[k];
					if ( remove_this_card ) pl.Deck.remove(k);
					matched_num++;
				}
			}
			for ( let k = 0; k < pl.DiscardPile.length; ++k ) {
				if ( card_ID === pl.DiscardPile[k].card_ID ) {
					...
					if ( remove_this_card ) pl.DiscardPile.remove(k);
					matched_num++;
				}
			}
			for ( let k = 0; k < pl.HandCards.length; ++k ) {
				if ( card_ID === pl.HandCards[k].card_ID ) {
					card = pl.HandCards[k];
					if ( remove_this_card ) pl.HandCards.remove(k);
					matched_num++;
				}
			}
			for ( let k = 0; k < pl.PlayArea.length; ++k ) {
				if ( card_ID === pl.PlayArea[k].card_ID ) {
					card = pl.PlayArea[k];
					if ( remove_this_card ) pl.PlayArea.remove(k);
					matched_num++;
				}
			}
			for ( let k = 0; k < pl.Aside.length; ++k ) {
				if ( card_ID === pl.Aside[k].card_ID ) {
					card = pl.Aside[k];
					if ( remove_this_card ) pl.Aside.remove(k);
					matched_num++;
				}
			}
			for ( let k = 0; k < pl.Open.length; ++k ) {
				if ( card_ID === pl.Open[k].card_ID ) {
					card = pl.Open[k];
					if ( remove_this_card ) pl.Open.remove(k);
					matched_num++;
				}
			}
		}
		...
		for ( let i = 0; i < G.Supply.Basic.length; ++i ) {
			...
			for ( let k = 0; k < spl.length; ++k ) {
				if ( card_ID === spl[k].card_ID ) {
					card = spl[k];
					if ( remove_this_card ) spl.remove(k);
					matched_num++;
				}
			}
		}
		...
		for ( let k = 0; k < G.TrashPile.length; ++k ) {
			if ( card_ID === G.TrashPile[k].card_ID ) {
				card = G.TrashPile[k];
				if ( remove_this_card ) G.TrashPile.remove(k);
				matched_num++;
			}
		}
